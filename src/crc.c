/*! \file crc.c
Модуль расчёта контрольной суммы блока
*/

#include "CRC.h"

//! Таблица для быстрого подсчета CRC16
...
uint16_t CRC16(void *Buffer, uint16_t Size) {
    ...
    while (Size--) {
        Result = (Result >> 8) ^ CRC16Table[(Result & 0x00FF) ^ *p++];
    }
...
uint8_t CheckCRC16(void *Buffer, uint16_t Size) {
    uint8_t *p = (uint8_t *)Buffer;
    Size -= 2;
    uint16_t CRC = ((uint16_t)p[Size + 1] << 8) | (uint16_t)p[Size];
    if (CRC == CRC16(Buffer, Size)) {
        return CRC_OK;
    }
    return CRC_ERROR;
}
...
uint16_t CRC16byStepFromFunctionRead(uint32_t Adr, uint32_t Size, uint16_t StartCRC16, TReadFlash ReadFlash) {
    uint16_t Result = StartCRC16;
    for (; Size; Adr++, Size--) {
        Result = (Result >> 8) ^ CRC16Table[ReadFlash(Adr) ^ (uint8_t)Result];
    }
    return (Result);
}
...
uint32_t CRC32byStep(void *Buffer, uint16_t Size, uint32_t StartCRC32, uint8_t isEndStep) {
    uint8_t *p = (uint8_t *)Buffer;
    uint32_t Result = StartCRC32;
    while (Size--) {
        Result = (Result >> 8) ^ Crc32Table[(*p++ ^ Result) & 0xFF];
    }
...
uint32_t CRC32byStepFromFunctionRead(uint32_t Adr, uint32_t Size, uint32_t StartCRC32, TReadFlash ReadFlash, uint8_t isEndStep) {
    uint32_t Result = StartCRC32;
    for (; Size; Adr++, Size--) {
        Result = (Result >> 8) ^ Crc32Table[(ReadFlash(Adr) ^ Result) & 0xFF];
    }
    if (isEndStep != 0) {
        Result ^= 0xFFFFFFFF;
    }
